...
void getTailPeaks(Int_t ch, Calibration &cal, Double_t searSigma, Double_t searPerc){
  /*
    What should go in:
  ...
  std::string outFile_str = CH_str+"_"+std::to_string(searSigma)+"_"+ std::to_string(searPerc)+"_grid.root";
...
void fitCh1(Double_t searSigma, Double_t searPerc){
  Calibration cal;
  ...
  getTailPeaks(9, cal, searSigma, searPerc);
}



void fitCh9(Int_t sig, Double_t perc){
  Calibration cal;
