...
std::string dName(std::string searString="_N_"){
  ...
  std::string dirName=folderName.substr(folderName.find(searString)+
                                      searString.size());
...
void basePh(Double_t tresh=0, Int_t rebin=10){

  Double_t saturationValue=14800;

  //std::string runName=CosmicRun1::names[runIndex];

  std::string runName=dName("CR2/");
  ...
  int gphotons(0);
  for(auto s:Glass){
    gphotons+=makePhotonHist(s, hg, 1.11, saturationValue, tresh);
  }
  ...
  int nphotons(0);
  for(auto s:noGlass){
    nphotons+=makePhotonHist(s, hn, 1.0, saturationValue, tresh);
  }
