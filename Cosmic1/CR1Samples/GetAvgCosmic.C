...
void GetAvgCosmic(){
  ...
  for(int fileIndex=0; fileIndex<CosmicRun1::names.size(); fileIndex++){
    std::string runName=CosmicRun1::names[fileIndex];
    ...
    for(auto ch:LXeutils::Channels){
      ...
      std::vector<Double_t> I, avgSig=getAvg(inFile, treeName,
                                          CosmicRun1::peakPositionMean[fileIndex],
                                          3*CosmicRun1::peakPositionVar[fileIndex]);
