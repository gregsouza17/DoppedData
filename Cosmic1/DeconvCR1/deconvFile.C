...
void deconvAFile(Int_t runIndex=0){
  ...
  std::string inFileName=CosmicRun1::names[runIndex]+"_avg.root";
  std::string foutName=CosmicRun1::names[runIndex]+"_deconved.root";
...
void deconvFile(){
  for(Int_t runIndex=0; runIndex<8; runIndex++){
    std::cout << runIndex <<" \n ";
    deconvAFile(runIndex);
  }
}
