...
void basePh(Double_t tresh=4000, Int_t rebin=10){

  Double_t saturationValue=14800;

  //std::string runName=CosmicRun1::names[runIndex];

  int num=monitorNumber("ring");
  std::string runName="Xe0to10ppm_tresh"+std::to_string( (int) tresh)+
    "_"+std::to_string(num);
  ...
  int gphotons(0);
  for(auto s:Glass){
    gphotons+=makePhotonHist(s, hg, 1.2, saturationValue, tresh);
  }
